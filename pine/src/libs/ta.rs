...
use crate::types::{
    ...
    DataType, Float, Int, PineClass, PineFrom, PineRef, PineStaticType, PineType,
...
impl<'a> SeriesCall<'a> for LineFromNaVal<'a> {
    fn step(
        ...
        if p.len() == 1 {
            ...
            move_tuplet!((x1, y1, x2, y2, xloc, _extend, _color, _style, _width) = p);
            let line = PerLine {
